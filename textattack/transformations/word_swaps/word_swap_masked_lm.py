...
class WordSwapMaskedLM(WordSwap):
    ...
    def _encode_text(self, text):
        ...
        # print("_encode_text:", text, encoding)
    ...
    def _bert_attack_replacement_words(
        ...
        # )
        target_ids_pos = list(
            range(masked_index, min(masked_index + len(word_tokens), masked_index + 3))
        )

        # print(masked_index + len(word_tokens), self.max_length, current_text.words[index])
        ...
        if not len(target_ids_pos):
            ...
            # print(target_ids_pos, current_text.words[index])
    ...
    def _get_transformations(self, current_text, indices_to_modify):
        ...
        if self.method == "bert-attack":
            ...
            with torch.no_grad():
                pred_probs = self._language_model(True, **current_inputs)[0][0]
